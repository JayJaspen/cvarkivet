...
type AnropsResultat<T> = { ok: true; data: T } | { ok: false; error: string };
...
async function anropaClaude<T>(
  ...
  try {
    ...
    if (start === -1 || slut === -1) {
      ...
      return { ok: false, error: 'AI-svaret gick inte att tolka.' };
    }

    return { ok: true, data: JSON.parse(innehall.slice(start, slut + 1)) as T };
  } catch (err) {
...
${
  ...
  return { ok: true, data: { score, motivation } };
}
